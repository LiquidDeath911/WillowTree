...
namespace WillowTree
{
    public partial class WillowTreeMain : Form
    {
        ...
        public WillowTreeMain()
        {
            ...
            CreatePluginAsTab("Challenge Logs", new ucChallenges());
        ...
        private void ConvertListForEditing(InventoryList itmList, ref List<WillowSaveGame.BankEntry> itmBank)
        {
            ...
            {
                List<int> itmBankValues = new List<int>() { itmBank[i].Quantity, itmBank[i].Quality, itmBank[i].Equipped, itmBank[i].Level, itmBank[i].IsJunk, itmBank[i].IsLocked };
        ...
        private void RepopulateListForSaving(InventoryList itmList, ref List<WillowSaveGame.BankEntry> itmBank)
        {
            ...
            foreach (InventoryEntry item in itmList.Items.Values)
            {
                ...
                itm.IsJunk = (byte)values[4];
                itm.IsLocked = (byte)values[5];
...
//            catch { MessageBox.Show("Couldn't save WSG"); }
        ...
        {
            // Send the OnGameSaving event before shutdown so that the locker tab will save the contents 
            // of the locker file.  Other custom tabs could also save their state in response to this.
