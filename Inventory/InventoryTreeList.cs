...
namespace WillowTree.Inventory
{
    public class InventoryList
    {
        ...
        public bool SaveToXml(string filename)
        {
            ...
            // Save to a temporary file so that if the operation fails the original file is not clobbered
            string tempFile = filename + ".tmp";
            if (!Util.RequestTempFile(tempFile))
                return false;
            ...
            catch (Exception ex)
            {
                MessageBox.Show(
                    "SaveToXml failed",
                    "Filename: \"" + tempFile + "\"\n\n" +
                    "Error encountered while attempting to write XML file.\n\n" +
                    ex.Message,
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Error);
            }

            // Replace the previous file with the temporary file
            if (operationSucceeded && Util.ReplaceFileWithTempFile(filename, tempFile))
                return true;

            // Clean up the temporary file if the operation failed and remnants of it remain
    ...
    public class InventoryTreeList
    {
        ...
        public bool ImportFromXml(string InputFile, int EntryType)
        {
            XmlDocument doc = new XmlDocument();
        ...
        public bool LoadFromXml(string InputFile, int EntryType)
        {
            bool operationSuccessful = false;

            Tree.BeginUpdate();
            this.Clear();
            operationSuccessful = ImportFromXml(InputFile, EntryType);
            Tree.EndUpdate();
