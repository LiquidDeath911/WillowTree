...
namespace WillowTree.Plugins
{
    public partial class ucLocker : UserControl, IPlugin
    {
        ...
        public void InitializePlugin(PluginComponentManager pm)
        {
            ...
            LockerTL.NavigationLayers = 1;
            LockerTL.IEComparisonEngine.ComparerIndex = 3;

            string lockerFilename = db.OpenedLockerFilename();
            if (!System.IO.File.Exists(lockerFilename))
            {
                MessageBox.Show("The last used locker file could not be found.  Reverting to the default locker file.");
        ...
        public bool LoadLocker(string InputFile)
        {
            bool operationSuccessful = false;

            LockerTL.Tree.BeginUpdate();
            LockerTL.Clear();
            operationSuccessful = LockerTL.ImportFromXml(InputFile, InventoryType.Any);
            LockerTL.Tree.ExpandAll();
        ...
        private void OpenLocker_Click(object sender, EventArgs e)
        {
            ...
            if (FromFile.ShowDialog() == DialogResult.OK)
            {
                // Save the current locker in case new items were added to it
                if (!LockerTL.SaveToXml(db.OpenedLockerFilename()))
                    return;

                try
                {
                    if (LoadLocker(FromFile.FileName()))
                        db.OpenedLockerFilename(FromFile.FileName());
                    // Save the settings so that if WT# crashes before shutdown the locker filename will
                    // not be forgotten.
                    GlobalSettings.Save();
                }
