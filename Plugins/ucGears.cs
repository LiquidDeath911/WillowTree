...
namespace WillowTree.Plugins
{
    public partial class ucGears : UserControl, IPlugin
    {
        ...
        public void InitializePlugin(PluginComponentManager pm)
        {
            ...
            switch (this.Text)
            {
                case "Weapons":
                    ...
                    GearTL.NavigationLayers = 1;
                    GearTL.IEComparisonEngine.ComparerIndex = 3;
                    this.gbGear.Text = "Weapon Backpack";
                    this.copyToBackpackToolStripMenuItem.Visible = false;
                    this.copyToBackpackToolStripMenuItem.Enabled = false;
                    this.newBankToolStripMenuItem.Visible = false;
                    this.newBankToolStripMenuItem.Enabled = false;
                    break;
                case "Items":
                ...
                case "Bank":
                    ...
                    this.copyToBankToolStripMenuItem.Enabled = false;
        ...
        private void Init()
        {
            ...
            {
                case "Weapons":
                    DoGearTabs(db.PartManager.WeaponPartCategories);//TODO RSM Push both Weapon/Item in array (db.WpnFile) so we don't have to reload
                    gearTextName = "Weapon";
                    gearVisibleLine = 15;
                ...
                case "Items":
                    DoGearTabs(db.PartManager.ItemPartCategories);
                    gearTextName = "Item";
                    gearVisibleLine = 17;
        ...
        private void DoGearTabs(Dictionary<string, List<string>> partCategories)
        {
            PartSelectorGear.Clear();
            Util.DoPartCategories(partCategories, PartSelectorGear);
        }
        ...
        private void NewBankWeapon_Click(object sender, EventArgs e)
        {
            GearTL.AddNew(InventoryType.Weapon);//Bank AddNewWeapon/Item
            GearTL.AdjustSelectionAfterAdd();
            GearTree.EnsureVisible(GearTree.SelectedNode);
        }
        private void NewBankItem_Click(object sender, EventArgs e)
        {
            GearTL.AddNew(InventoryType.Item);//Bank AddNewWeapon/Item
            GearTL.AdjustSelectionAfterAdd();
            GearTree.EnsureVisible(GearTree.SelectedNode);
        }
        ...
        private void PartSelectorGear_SelectionChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string upath = PartSelectorGear.SelectedNode.Parent.GetKey() + '.' + PartSelectorGear.SelectedNode.GetText();
                PartInfo partInfo;
                if (db.PartManager.PartsDb.TryGetValue(upath, out partInfo))
                    PartInfoGear.Lines = partInfo.GetLines().ToArray();
            }
            catch { }
        }
