...
//                Color.FromArgb(3111167),
        ...
        static public bool Save()
        {
            ...
            try
            {
                using (XmlTextWriter gs = new XmlTextWriter(tempFile, Encoding.UTF8))
                {
            ...
            catch (Exception ex)
            {
                MessageBox.Show(
                    "Save failed",
                    "Filename: \"" + tempFile + "\"\n\n" +
                    "Error encountered while attempting to write XML file.  Your settings were not saved properly.\n\n" +
                    ex.Message,
